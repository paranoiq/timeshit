...
// 14. pause closes the open record with the `paused` trigger (semantically same as end)
[$app, $store, $clock] = newApp('2026-05-09 10:00');
$app->run(['ts', 'track', 'ABC-1']);
$clock->advance('+30 minutes');
Assert::same(0, $app->run(['ts', 'pause']));
Assert::same('paused', $store->load()[0]->endTrigger);
...
// 16. resume opens a new record cloned from the latest closed (issue/type/branch/repo preserved)
[$app, $store, $clock] = newApp('2026-05-09 10:00');
$app->run(['ts', 'track', 'ABC-1', 'doc']);
$clock->advance('+30 minutes');
$app->run(['ts', 'pause']);
...
// 17. resume with comment puts the text on the new record (not appended to the prior one)
[$app, $store, $clock] = newApp('2026-05-09 10:00');
$app->run(['ts', 'track', 'ABC-1']);
$clock->advance('+30 minutes');
$app->run(['ts', 'pause']);
...
$items = $store->load();
Assert::same('', $items[0]->comment);
Assert::same('after lunch', $items[1]->comment);
...
// 19. resume when the latest record is open errors
