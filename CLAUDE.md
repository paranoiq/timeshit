...
config/                 NEON config (`config.neon` committed, `secrets.neon` gitignored)
  config.neon           committed — non-secret keys: `youtrackBaseUrl`, `timezone`, `allowedTypes`, `defaultTrackType`, `defaultDayType`, `interruptionTypes`
...
- `php timeshit.php interrupt <issue> [<type>]` — like `track` (same issue/type rules; default type is `defaultTrackType`), but unconditionally tags the seam with `'interrupted'` whenever an open record exists, regardless of whether the open record is `defaultTrackType` or the new type is in `interruptionTypes`. Used to declare an interruption explicitly when the type-based auto-detection wouldn't fire (e.g. interrupting a `Documentation` record, or interrupting with an `Implementation` record). When no record is open, falls back to plain `'manual'` (nothing to interrupt). Pairs with `done` to auto-resume.
...
- `php timeshit.php pause [<comment>]` — closes the currently open tracking record (`endTrigger="paused"`, `status="paused"` via the store auto-mapping) AND immediately appends a new **open** "break" record (`issueId=""`, `type=""`, no `branch`, `repo=""`, `startTrigger="paused"`, `status="untracked"`) representing the time you're away. The optional `<comment>` lives on the new break record (NOT appended to the closed tracking record) — use it for the reason for the break ("lunch", "standup", …). Errors when no tracking entry is open. A second `pause` while already paused is a silent no-op (the open break and the new break record have the same identity tuple, so the store detects the match and skips). The break record is closed later by `resume` / `done` / `track` / `end` / etc. — whichever command produces the next seam — and that closing trigger becomes its `endTrigger`.
- `php timeshit.php resume [<comment>]` — append a new open record cloned from a previous closed record (same `issueId`/`branch`/`repo`/`type`), with `startTrigger="resumed"`. The target is picked by scanning backwards for the most recent record with `status="paused"` (covers both manual `pause` and auto-paused-by-interruption — both are tagged `paused` by the stores), so the interrupted/paused work always wins over a more recent unrelated `end`; falls back to the most recent closed non-untracked record when nothing is paused. Optional `<comment>` becomes the new record's comment. Errors when no records exist OR when the latest record is open AND its status is not `untracked` — in other words, an open break record from `pause` is NOT an error: `resume` closes it (with `endTrigger="resumed"`) before appending the new tracking record.
- `php timeshit.php skip <span>` — close the currently open record at `now - <span>` and immediately open a new one cloned from it (same `issueId`/`branch`/`repo`/`type`) starting at `now`. Both `endTrigger` of the closed segment and `startTrigger` of the new one are recorded as `"skipped"`. Used when you forgot to `pause` during a break: the `<span>` is the length of the unrecorded gap (e.g. `45m` for a lunch break). `<span>` is parsed by `Resolver::parseSpan` (same `d`/`h`/`m` grammar as `before` / `after`). Errors when no entry is open or the span would push the close time at or before the open record's `startedAt`.
- `php timeshit.php grab <issue> <span> [<type>]` — like `skip`, but the `<span>`-long hole is **filled** with a closed record for `<issue>` instead of left empty. Three records are written in one save: (1) the original open record closed at `now - <span>` with `endTrigger="grabbed"`; (2) a new closed record for `<issue>` from `now - <span>` to `now` with `startTrigger="grabbed"` / `endTrigger="grabbed"`; (3) a continuation cloned from the original (same `issueId`/`branch`/`repo`/`type`) starting at `now` with `startTrigger="grabbed"`. `<issue>` is validated by `Resolver::requireIssueId` (must match `[A-Za-z]+-\d+`); `<type>` follows `track`'s rules (default `Implementation`, optional case-insensitive resolution against `Config::$allowedTypes`). The grabbed record has `repo=""` and no `branch` field (same shape as `track`). Used when you forgot to `track` an interruption: e.g. `ts grab XYZ-12 30m` after spending half an hour on a different ticket without switching. Same span-too-large guard as `skip`.
...
- `php timeshit.php done [<comment>]` — like `end` (closes the open entry with `endTrigger="done"` and an optional appended `<comment>`), then auto-resumes the most recent record with `status="paused"` (scanning backwards from before the just-closed record). When such a record exists, a new open record is appended cloned from it with `startTrigger="resumed"`; when none exists, `done` behaves exactly like `end`. Both manual `pause` and the interruption flow leave records with `status="paused"`, so `done` brings either kind of paused work back. Errors when no entry is open.
...
- `tests/CmdLocalEdit.phpt` — `at` / `before` / `after` / `skip` / `grab`: `at HH:MM` on open shifts `startedAt`, on closed shifts `endedAt`, cancel-on-`n` leaves the store untouched, no-op on same value, non-positive-duration guard; `before` captures `origStartedAt` once and only once, adjacency drags the prior record's `endedAt`, malformed span rejection; `after` open-record rejection; `skip` two-record split + span-too-large guard; `grab` three-record write with default + explicit type, invalid issue rejection, no-open / span-too-large guards.
...
- `tests/CmdLocalCombo.phpt` — multi-command flows: `track`→`pause`→`resume`→`end` (now produces three records — paused tracking, closed break, ended-resumed segment); `track`→`switch`→`end` (multi-type segments); `track`→`end`→`track`→`before` triggering adjacency (prior `endedAt` and new `startedAt` shift together with `origEndedAt`/`origStartedAt` capture); `track`→`comment`→`switch` (comment stays on the first segment); `track`→`grab`→`end` (interruption flow yielding three records); `end`→`comment` lands on the freshly-closed record; `at` after `end` followed by `track` proves `origEndedAt` persists across subsequent writes; `track`→`before`→`end`→`at` accumulates both `origStartedAt` and `origEndedAt` on a single record.
- `tests/CmdLocalInterruption.phpt` — the auto-pause/auto-resume flow that fires when an open `defaultTrackType` record is replaced by a track of an `interruptionTypes` member: tags both ends with `'interrupted'` and sets `status='paused'` on the closed record; `done` closes the interrupting record (`endTrigger='done'`) and auto-resumes the most recent `status='paused'` predecessor (`startTrigger='resumed'`, cloned issue/branch/repo/type); `end` or another `track` over the interrupting record leaves the original paused; `resume` (status-based scan) recovers it; negative cases — non-default open type, non-interruption new type — fall back to the regular `'manual'` track-replace and leave `status='new'`; `done` with no paused predecessor behaves like `end`; manual `pause`+`done` also auto-resumes (since both pause flavours share `status='paused'`); manual `pause`+`resume` regression preserved. Plus the explicit `interrupt` command: forces the seam tag regardless of types, pairs with `done` to auto-resume non-default-type records, accepts an explicit `<type>`, falls back to `'manual'` when no record is open, rejects invalid issue ids. Plus a status-field lifecycle block — fresh records load as `'new'`, manual `pause` flips to `'paused'`, `resume` opens a fresh `'new'` continuation, `end`/`switch`/track-replace all leave `'new'`, and legacy records lacking the field decode as `'new'`.
...
- `Config` — value object with `youtrackBaseUrl`, `youtrackToken`, `timezone`, `allowedTypes`, `defaultTrackType`, `defaultDayType`, `interruptionTypes`. Construct directly in tests (`new Config(...)`); production loads it via `Config::load($rootDir)`.
...
`startRecord` (called by `track`, `interrupt`, and `checkout`) detects the *interruption* case via two channels: (1) the **type-based** path — when the latest record is open AND its `type === Config::$defaultTrackType` AND the new record's resolved type is in `Config::$interruptionTypes`; and (2) the **explicit** path — when the caller passes `forceInterruptIfOpen: true` (only `cmdInterrupt` does). Either path promotes the trigger string passed into the store to `'interrupted'` instead of the call site's default (`'manual'` for `track` / `interrupt`, `'checkout'` for the git hook). The store then uses that string for the closed record's `endTrigger` AND the new record's `startTrigger`, AND independently bumps the closed record's `status` to `'paused'` (via `pausesViaTrigger`). `cmdSwitch`, `cmdResume`, `cmdGrab` call `RecordStore::track` directly with their own triggers (`'switched'` / `'resumed'` / `'grabbed'`) and never produce `'interrupted'`. When `interrupt` is used with no record currently open, neither path fires (no seam to tag) and the new record is simply started with `startTrigger='manual'` and `status='new'`.

`done` (in `App::cmdDone`) is a two-step write: first `RecordStore::endOpen($now, 'done', $comment)` closes the open record (errors when none is open, like `end`), then it scans the resulting list backwards from index `count-2` looking for the most recent record with `status === 'paused'`. When found, it appends a clone of that record (same `issueId`/`branch`/`repo`/`type`, `startTrigger='resumed'`) — same shape as a manual `resume`. When no paused predecessor exists, `done` is identical to `end`. Because both manual `pause` and the auto-pause-on-interruption produce records with `status='paused'`, **`done` resumes either kind of paused work**; the `endTrigger` on the closed tracking record (`'paused'` vs `'interrupted'`) is preserved for forensics but no longer drives detection. Note that `pause` *also* leaves an `untracked` break record between the paused tracking and the next event — `done`'s scan skips that since it's looking for `status='paused'` (not `'untracked'`).
